...
from new_J_net.layers import ConvOffset2D
...
class Dense_decoder(nn.Module):
    def __init__(self, encoder_channel, out_channel, drop_rate, norm_type):
        ...
        self.refine1 = nn.Conv2d(16 + encoder_channel, 20, 3, 1, 1)
        self.tanh = nn.Tanh()
...
class Encoder(nn.Module):
    def __init__(self, input_channel, drop_rate, norm_type):
        ...
        self.feature0.add_module("conv0", nn.Conv2d(input_channel, inner_channels, kernel_size=3, stride=2, padding=1,
                                                    bias=False))
        if norm_type == 'IN':
...
class AtJF_net(nn.Module):
    def __init__(self, drop_rate, norm_type):
        super(AtJF_net, self).__init__()
        self.encoder_At = Encoder(input_channel=3, drop_rate=drop_rate, norm_type=norm_type)
        self.decoder_A = Dense_decoder(encoder_channel=3, out_channel=3, drop_rate=drop_rate, norm_type=norm_type)
        self.decoder_t = Dense_decoder(encoder_channel=3, out_channel=1, drop_rate=drop_rate, norm_type=norm_type)

        self.encoder_J = Encoder(input_channel=3, drop_rate=drop_rate, norm_type=norm_type)
        self.decoder_J = Dense_decoder(encoder_channel=3, out_channel=3, drop_rate=drop_rate, norm_type=norm_type)

        self.encoder_F = Encoder(input_channel=9, drop_rate=drop_rate, norm_type=norm_type)
        self.decoder_F = Dense_decoder(encoder_channel=9, out_channel=2, drop_rate=drop_rate, norm_type=norm_type)

    def forward(self, x):
        At_x1, At_x2, At_x4 = self.encoder_At(x)
        A = self.decoder_A(x, At_x1, At_x2, At_x4)
        t = self.decoder_t(x, At_x1, At_x2, At_x4, activation='sig')
        t1 = torch.abs(t) + (10 ** -10)
        t1 = t1.repeat(1, 3, 1, 1)
        J_re = (x - A * (1 - t1)) / t1

        J_x1, J_x2, J_x4 = self.encoder_J(x)
        J = self.decoder_J(x, J_x1, J_x2, J_x4)

        x = torch.cat([x, J_re], 1)
        x = torch.cat([x, J], 1)
        F_x1, F_x2, F_x4 = self.encoder_F(x)
        w = self.decoder_F(x, F_x1, F_x2, F_x4)
        w_re, w_en = w.split(1, 1)
        J_f = J_re * w_re + J * w_en

        return A, t1, J_re, J, w_re, w_en, J_f
