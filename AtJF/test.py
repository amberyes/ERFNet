...
parser.add_argument('-weight', help='The loss weight', default=[1, 1, 1, 1, 1], type=list)
...
# 加载训练好的模型
file_path = find_pretrain('AtJF_result')
model_path = file_path + '/AtJF_model.pt'
...
if category == 'NYU':
    ...
    test_gt_path = data_path + '/data/nyu/test_gt_patch/'
    test_t_path = data_path + '/data/nyu/test_t_patch/'
    test_A_path = data_path + '/data/nyu/test_A_patch/'
    test_path_list = [test_hazy_path, test_gt_path, test_t_path, test_A_path]
else:
    test_hazy_path = data_path + '/data/cut_ntire_cycle/test_hazy/'
    test_gt_path = data_path + '/data/cut_ntire_cycle/test_gt/'
    test_path_list = [test_hazy_path, test_gt_path]
...
for name, haze_image, gt_image, A_gt, t_gt in test_data_loader:
    net.eval()
    with torch.no_grad():
        ...
        loss_image = [A, A_gt, t, t_gt, J_reconstruct, gt_image]
