...
def create_test_results_folder(file_path, excel_row):
    """
    创建保存测试结果的文件夹
    :return:
    """
    ...
    if not os.path.exists(save_path):
        ...
        os.makedirs('{}/image'.format(save_path))

    # 创建用于保存测试指标的表格文件
    excel_test_line = 1
    excel_save = './{}/test_result_{}.xls'.format(save_path, local_time)
    f, sheet_test = init_test_excel(row=excel_row)
    save_path = '{}/image'.format(save_path)
    return save_path, f, sheet_test, excel_test_line, excel_save
...
if __name__ == '__main__':
    ...
    parser.add_argument('-weight', help='The loss weight', default=[1], type=list)
    parser.add_argument('-excel_row', help='The excel row',
                        default=["num", "A", "beta", "MAE_J"], type=list)
    ...
    # 加载训练好的模型
    file_path = find_pretrain('J_result')
    model_path = file_path + '/J_model.pt'
    net = torch.load(model_path)
    net = net.cuda()
    loss_net = test_loss_net(pixel_loss='MAE', re_weight=1).cuda()
    ...
    if Is_train_result:
        if category == 'NYU':
            ...
            output_path = [data_path + '/data/nyu/train_en_patch/', data_path + '/data/nyu/val_en_patch/',
                           data_path + '/data/nyu/test_en_patch/']
        else:
            ...
            for name, haze_image in test_data_loader:
                ...
                with torch.no_grad():
                    ...
                    for j in range(len(name)):
                        im_output_for_save = get_image_for_save(J[j])
        ...
        if category == 'NYU':
            ...
            test_re_path = data_path + '/data/nyu/test_re_patch/'
            test_path_list = [test_hazy_path, test_gth_path, test_re_path]
        else:
        ...
        # 创建用于保存测试结果的文件夹
        save_path, f, sheet_test, excel_test_line, excel_save= create_test_results_folder(file_path, excel_row)

        # 创建图像数据加载器
        if category == 'NYU':
            test_data = train_DataSet(transform, path=test_path_list)
        else:
        ...
        for name, haze_image, gt_image, re_image in test_data_loader:
            net.eval()
            with torch.no_grad():
                haze_image = haze_image.cuda()
                gt_image = gt_image.cuda()
                re_image = re_image.cuda()
                J = net(haze_image)
                loss_image = [J, gt_image, re_image]
                temp_loss = loss_net(loss_image, weight)
                if Is_save_image:
