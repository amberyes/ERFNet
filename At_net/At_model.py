...
class Dense_decoder(nn.Module):
    def __init__(self, out_channel, drop_rate, norm_type):
        ...
        self.refine1 = nn.Conv2d(19, 20, 3, 1, 1)
        self.tanh = nn.Tanh()
...
class Encoder(nn.Module):
    def __init__(self, drop_rate, norm_type):
        ...
        self.feature0.add_module("conv0", nn.Conv2d(3, inner_channels, kernel_size=3, stride=2, padding=1, bias=False))
        if norm_type == 'IN':
...
class At_net(nn.Module):
    def __init__(self, drop_rate, norm_type):
        ...
        self.encoder_1 = Encoder(drop_rate=drop_rate, norm_type=norm_type)
        self.decoder_A = Dense_decoder(out_channel=3, drop_rate=drop_rate, norm_type=norm_type)
        self.decoder_t = Dense_decoder(out_channel=1, drop_rate=drop_rate, norm_type=norm_type)
